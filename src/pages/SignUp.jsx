...
export default function SignUp({ isAdmin = false }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await signup(email, password, name || "User", isAdmin ? "admin" : "customer");
      showToast(`Account created successfully! Welcome to Nikdel${isAdmin ? " Admin" : ""}.`, "success");
      ...
      if (backendMessage.toLowerCase().includes("already exists")) {
        ...
        setError(backendMessage || "Failed to create account. Please check details.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      showToast("Account created successfully via Google! Welcome to Nikdel.", "success");
      navigate("/");
    } catch (err) {
      console.error(err);
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-950 to-brand-950 p-10 text-white flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10 my-8 space-y-6">
          <h2 className="text-3xl font-extrabold tracking-tight leading-tight">
            {isAdmin ? "Manage Your Premium Store." : "Discover the Art of Premium Shopping."}
          </h2>
      ...
      <div className="p-8 sm:p-10 flex flex-col justify-center space-y-6 bg-white">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-405 font-bold">
            {isAdmin ? "Register to access the admin portal" : "Register in seconds to unlock checkouts"}
          </p>
        </div>

        {error && (
          <div className="bg-red-50 border border-red-100 text-red-650 font-semibold p-4 rounded-xl text-xs">
            {error}
          </div>
        )}
          ...
          <div className="flex flex-col gap-1.5 relative">
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all"
              />
            </div>
          </div>
        ...
        <p className="text-center text-xs text-slate-450 font-semibold">
          ...
          <Link to={isAdmin ? "/admin/login" : "/login"} className="text-brand-600 hover:underline">
            Sign In
          </Link>
        </p>
      </div>
